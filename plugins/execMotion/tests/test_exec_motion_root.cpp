...
//
// What it answers is where the body is, under the intake policy the clip states
// -- `openstrata::motion::RootMotionIntake`, the library's own enum, spelled as
// a token. Three things are measured here that no earlier suite could:
//
...
// for. A cleared `openstrata::motion::RootMotion` is `ignore`'s own legitimate
// answer, bit for bit, so a refusal that produced one would hand a misspelled
// `passthrough` the behaviour of a deliberate `ignore` -- indistinguishable to
// anyone not reading `TfError`s. The `ignore` block and the unrecognized-token
// block sit next to each other below, and the pair is the assertion.
//
...
// it: a cleared `openstrata::motion::RootMotion` is `ignore`'s own legitimate
// answer, bit for bit, so a refusal that produced one would be
// indistinguishable from a deliberate "this clip does not place the body" for
// anyone not reading `TfError`s. An empty `VtValue` is the one shape no
// computation here ever produces as an answer.
void
...
// joints the fixtures name, at identity, with the hips at the origin. So the
...
PriorPose(double timestamp)
{
    ...
    for (const openstrata::motion::HumanJoint joint : {openstrata::motion::HumanJoint::Hips,
...
TestAClipWithAPolicy(const std::string& fixture)
{
    ...
    // the pose is empty, and an empty pose has no root.
    {
        ...
        // `openstrata::motion::RootMotion` legitimately arises -- beside
        // `ignore` and a pose that states no position -- which is why a refusal
        // cannot be spelled that way.
    ...
    // reached `openstrata::motion::RootMotionIntake`: the same two poses under
    // the library's default produce a velocity, asserted in the no-policy suite
    ...
    // A cleared `openstrata::motion::RootMotion` rather than a zeroed position:
    // every presence flag is false, which is what "this clip's placement is not
    // the capture's to decide" looks like downstream. A zero position with
    ...
    // cleared `openstrata::motion::RootMotion` is what the block immediately
    // above returns for a deliberate `ignore`, bit for bit, so a refusal
    // producing one would hand a misspelled `passthrough` the behaviour of an
    // `ignore` nobody asked for. The two blocks are next to each other on
    // purpose.
...
TestAClipWithNoPolicy(const std::string& fixture)
{
    ...
    const openstrata::motion::MotionPose prior = PriorPose(kSecond - kStep);
    {
        ...
        // `openstrata::motion::LiveCaptureConfig`'s own default is
        // `DeriveVelocity`, and that is what a clip stating nothing gets. The
        // assertion is the same shape as the filter suite's 6 Hz: the *other*
        // two policies are distinguishable here, so this would not pass if the
        // bundle had picked a default of its own -- `Passthrough` derives
        // nothing and `Ignore` clears the position.
