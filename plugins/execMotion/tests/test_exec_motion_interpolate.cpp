...
// A pose a driver's buffer would hold: the four joints the fixture names, the
...
TestAHistoryIsSampledAtTheEvaluatedInstant(const std::string& fixture)
{
    ...
    // cannot prevent here, because the substitution never reaches a callback.
    {
        ...
        // type 'openstrata::motion::MotionClip'; got
        // 'openstrata::motion::MotionPose'".
    ...
    // that the key's own callback set none.
    {
        ...
        assert(ResultAt(view, kInterpolated).status ==
               openstrata::motion::PoseSampleStatus::Sampled);
        mark.Clear();
    }
...
TestTwoOverridesOfTwoKeysInOneCall(const std::string& fixture)
{
    ...
    // The previous frame's answer: the four joints at identity, the hips at the
