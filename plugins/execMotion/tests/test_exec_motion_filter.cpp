...
//
// The four joints are the ones the fixtures name, all at identity, with the hips
...
TestAClipWithAPolicy(const std::string& fixture)
{
    ...
    // `motion.priorPose` forwards the clip's own pose, so the filter smooths
    // the pose against itself: `dt` is zero, `openstrata::motion::PoseFilter`
    // reseeds and returns its argument. Nothing in the bundle special-cases
    // this; it is the library's documented answer to a non-increasing
    // timestamp.
    {
        ExecUsdCacheView view = system.Compute(request);
        ...
        assert(NearlyEqual(AngleDegrees(RotationOf(sampled, openstrata::motion::HumanJoint::Head)),
                           45.0,
                           1e-2) &&
...
TestAClipWithNoPolicy(const std::string& fixture)
{
    ...
    // `openstrata::motion::PoseFilter::Options`' own default is 6 Hz, and that
    // is what a clip stating nothing gets. The negative half is the one that
    // matters: the same step at the policy fixture's 12 Hz lands somewhere
    // else, so this assertion would not pass if the bundle had picked a default
    // of its own or carried the last clip's.
