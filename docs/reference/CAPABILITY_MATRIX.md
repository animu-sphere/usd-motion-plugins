...
Status (2026-09-19): **the core value types, sampling, recording, retargeting
and authoring a motion stage are implemented** (`motionCore`,
...
| Replaying a recorded trace into a motion stage (`motion_record`): intake policies, a simulated delivery lag, trace normalisation, the session's provenance in `customData.source` | supported — `motion_record_replay`, which opens the stage through OpenUSD and resolves it through a `UsdSkelSkeletonQuery`; channels and look-at targets are reported on stderr, not authored (USD §4.3 names the channel prim; nothing authors it until the reading half) | [MOTION §10](../design/MOTION_CONTRACT.md#10-recording-and-the-trace-format); [USD §2–§5](../design/USD_MAPPING.md#2-the-standalone-motion-stage) | — (imported 2026-09-19, ahead of its release) | v0.3.0 |
| Authoring a motion stage: `/Animation/{Skeleton,Body}`, 30 time codes per second, identity `scales`, `motion:timeCodesPerSecond`, `customData.motion` | supported — `motionUsd_unit`, which opens each stage through OpenUSD and checks what UsdSkel resolves; channels and look-at targets are reported, not authored (USD §4.3 names the channel prim; nothing authors it until the reading half) | [USD §2–§5](../design/USD_MAPPING.md#2-the-standalone-motion-stage) | — (imported 2026-09-19 from `usd-vrm-plugins` `motion_capture`) | v0.1.0 |
