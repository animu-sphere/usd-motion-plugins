...
using openstrata::motion::CanonicalBasis;
using openstrata::motion::ComposeSourceRotation;
using openstrata::motion::ConversionRefusal;
using openstrata::motion::ConvertPosition;
using openstrata::motion::ConvertRotation;
using openstrata::motion::ConvertSourceToCanonical;
using openstrata::motion::MakeCanonicalBasis;
...
WorldRest(const openstrata::motion::CanonicalRestPose& rest, openstrata::motion::HumanJoint bone)
{
...
TestProvenance()
{
    ...
    assert(result.animation.samples[0].metadata == openstrata::motion::SourceMetadata());
}
...
{
    for (std::size_t index = 0; index < openstrata::motion::ConversionRefusalCount; ++index)
    {
        assert(!openstrata::motion::ConversionRefusalName(static_cast<ConversionRefusal>(index)).empty());
    }
}
