                         ...
                         const SourceProfile& profile)
{
    ...
    for (std::size_t frame = 0; frame < animation.frameCount; ++frame)
    {
        ...
        // `pose.metadata` stays the default, which is how a sample says it
        // recorded nothing about where (MOTION_CONTRACT.md §5.1). It cannot
        // vary within a clip, the animation carries it once, and a per-sample
        // copy is the cost SourceProvenance.h argues against paying.
