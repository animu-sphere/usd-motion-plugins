...
"""Author the motion corpus: the recorded capture traces motionRecording replays.

Why these are synthesised rather than recorded from a real capture rig: the
avatar corpus of usd-vrm-plugins, where they were first written, is
licence-gated, and a motion corpus sourced from a commercial capture SDK or a
performer would inherit exactly the same problem -- a fixture nobody outside
the project can redistribute is a fixture CI cannot run. These traces are
generated by closed-form maths in this file, so they are Apache-2.0 like the rest of the repo, byte-stable
...
The output must match motionRecording's own writer byte for byte; the corpus test
(`motionRecording_corpus`) enforces that. Run:

    python libs/motionRecording/tools/generate_traces.py
...
# Must equal motionRecording's CaptureTraceFormatVersion. The corpus test re-emits
...
#
# This is a second copy of motionCore's HumanJoint enum, and the C++ side stays
# the single source of truth: motionRecording_corpus re-emits every committed trace
...
def render(frames: list[Frame], provider: str, protocol: str, source_id: str,
    ...
    """Emit the trace exactly as motionRecording's WriteCaptureTrace would."""
...
def main() -> int:
    ...
    if drifted:
        ...
        print("run: python libs/motionRecording/tools/generate_traces.py",
              file=sys.stderr)
        return 1
